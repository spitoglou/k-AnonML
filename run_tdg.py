#!/usr/bin/env python3
"""
Top-Down Greedy (TDG) k-Anonymity Algorithm Runner

This script provides a clean interface to run the TDG k-anonymity algorithm
on arbitrary datasets using the original implementation.

Usage:
    python run_tdg.py input.csv output.csv --k 5 [options]

Example:
    python run_tdg.py datasets/adult/adult.csv adult_tdg_k5.csv --k 5
"""
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='Run Top-Down Greedy k-anonymity algorithm',
...
def print_configuration(args, config):
    ...
    print("=" * 60)
    print("TOP-DOWN GREEDY K-ANONYMITY ANONYMIZATION")
    print("=" * 60)
    print(f"Input file: {args.input_file}")
    print(f"Output file: {args.output_file}")
    print(f"Dataset: {config['name']}")
    print(f"Algorithm: Top-Down Greedy")
...
def main():
    ...
    print(f"Running Top-Down Greedy k-anonymization (k={args.k})...")
    start_time = time.time()
    
    try:
        # Call the original TDG implementation
        anonymized_data = tdg_get_result_one(
            att_trees=None,  # TDG doesn't need att_trees
            data=data,
            k=args.k,
            path='.',
        ...
        # TDG doesn't return NCP and runtime separately
        ncp = 0.0
        ...
        print("=" * 60)
        print("Top-Down Greedy k-anonymization completed successfully!")
